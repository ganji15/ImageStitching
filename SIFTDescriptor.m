...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        img_dx = filter2([-1 0 1;-1 0 1;-1 0 1], currentImage, 'same');
        
        % gradients in y direction.
        img_dy = filter2([-1 -1 -1;0 0 0;1 1 1], currentImage, 'same');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        grad_theta{scale} = atan(img_dy ./ img_dx);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for y = 1 : num_histograms
            for x = 1 : num_histograms
                x_lo = (x - 1) * pixelsPerHistogram + 1;
                x_hi = x * pixelsPerHistogram;
                y_lo = (y - 1) * pixelsPerHistogram + 1;
                y_hi = y * pixelsPerHistogram;
                
                [histogram, ~] = ComputeGradientHistogram(num_angles, ...
                                                     patch_mag(y_lo : y_hi, x_lo : x_hi), ...
                                                            patch_theta(y_lo : y_hi, x_lo : x_hi));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [M, N] = size(gradient_magnitudes);
    for i = 1 :  num_bins
        hist_i = 0;
        for y = 1 : N
            for x = 1 : M
                if (abs(gradient_angles(x, y) / angle_step - (i - 1)) < 1) ||...
                        (abs(gradient_angles(x, y) / angle_step - (i - 1)) > (num_bins - 1))
                   hist_i = hist_i +  abs(cos(gradient_angles(x, y) - angles(i))) * gradient_magnitudes(x, y);
                end
            end
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    max_idx = 1;
    for i = 2 : num_bins
        if histogram(i) > histogram(max_idx)
            max_idx = i;
        end
    end
    
    direction = angles(max_idx);
