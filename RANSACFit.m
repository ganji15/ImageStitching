...
%   
    ...
    if N<3
        ...
        disp('not enough matches to produce a transformation matrix')
        H = eye(3);
        return
    end
    ...
    if sum(sum((H - eye(3)).^2)) == 0,
        H = RANSACFit(p1, p2, match, maxIter, seedSetSize, maxInlierError, goodFitThresh * 0.8);
        disp('No RANSAC fit was found, try again')
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    N = size(match,1);
    match_points_1 = [pt1(match(:, 1), :)'; ones(1, N)];
    match_points_2 = [pt2(match(:, 2), :)'; ones(1, N)];
    dists = sqrt( sum((H * match_points_1 - match_points_2).^2));
    dists = dists';
